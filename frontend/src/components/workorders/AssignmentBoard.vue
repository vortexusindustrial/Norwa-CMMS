      ...
      <tbody>
        ...
        >
          ...
          <td class="px-4 py-2.5 text-gray-700">
            {{ projectName(wo.projectId) }}
          </td>
          <td class="px-4 py-2.5">
            <span
              class="rounded-full px-2 py-0.5 text-xs font-medium"
              :class="PRIORITY_META[wo.priority].badgeClass"
            >
          ...
          <td class="px-4 py-2.5">
            <select
              ...
              @change="
                store.assignTechnician(wo.id, $event.target.value || null)
              "
            >
            ...
            <div v-else class="flex items-center gap-2">
              <span class="text-gray-900">{{
                technicianById(wo.technicianId)?.name ?? "Unassigned"
              }}</span>
              <button
                type="button"
